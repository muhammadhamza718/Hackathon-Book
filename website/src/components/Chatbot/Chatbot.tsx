...
export default function Chatbot() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "1",
      text: "Hello! I am your Physical AI assistant. Ask me anything about the course!",
      sender: "bot",
    },
  ]);
      ...
      if (selection && selection.toString().trim().length > 0) {
        ...
        try {
          const response = await fetch(
            "https://muhammadhamza7718-physical-ai-backend.hf.space/api/ask-selection",
            {
              method: "POST",
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const response = await fetch(
        "https://muhammadhamza7718-physical-ai-backend.hf.space/api/ask",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
